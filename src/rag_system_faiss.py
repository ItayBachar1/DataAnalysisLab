...
qa_df = pd.read_csv("/content/synthetic_qa_pairs.csv")
...
# Ensure all values in 'description' are strings, replace missing values with a placeholder
...
# Load your synthetic QA dataset
qa_df = pd.read_csv("/content/synthetic_qa_pairs.csv")
questions = qa_df['synthetic_question'].tolist()
reference_answers = qa_df['synthetic_answer'].tolist()
...
def evaluate_retrieval_metrics(questions, reference_answers, top_k=TOP_K):
    ...
    for question, reference in zip(questions, reference_answers):
        ...
        # Calculate recall at K (1 if any relevant item found, otherwise 0)
...
# Example usage: Evaluating the retrieval metrics
evaluate_retrieval_metrics(questions[:10], reference_answers[:10])  # Run on a subset for testing