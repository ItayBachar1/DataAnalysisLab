...
# check for missing values with expanded conditions and handling for nested structures
...
def fill_missing_values(row):
    ...
    if pd.isnull(row['first_review']):
        row['first_review'] = pd.NaT
    if pd.isnull(row['last_review']):
        row['last_review'] = pd.NaT
...
# Handle 'first_review' and 'last_review' specifically
...
# Convert the dataset to a DataFrame from a specific split
df = dataset['train'].to_pandas()
...
# Calculate fraction to achieve roughly 25 MB (try with 0.2 or adjust as needed)
sample_fraction = 0.18  # Adjust this value if needed

# Create a random sample
sampled_df = df.sample(frac=sample_fraction, random_state=42)

# Save the sampled DataFrame to CSV
sample_csv_path = 'sampled_airbnb_data.csv'
sampled_df.to_csv(sample_csv_path, index=False)
print(f"Sampled CSV file saved at: {sample_csv_path}")

from google.colab import files

# Download the CSV file
files.download(sample_csv_path)
...
# Filter by Numeric Fields (e.g., Beds, Price)
